using PG.Core.DBBase;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace PG.DBClass.ProductionDC
{
    [DBTable(Name = "PROD_ITEM_CONVERSION_BATCH")]
    public partial class dcPROD_ITEM_CONVERSION_BATCH : DBBaseClass, INotifyPropertyChanged
    {
        #region private members

        private int m_CONV_BATCH_ID = 0;
        private int m_CONVERSION_ID = 0;
        private string m_CONVERSION_NO = string.Empty;
        private int m_ITEM_ID = 0;
        private int m_DEPT_ID = 0;
        private int m_STLM_ID = 0;
        private string m_PROD_BATCH_NO = string.Empty;
        private decimal m_CLOSING_QTY = 0;
        private int m_UOM_ID = 0;
        private decimal m_ITEM_QTY = 0;
        private int m_SL_NO = 0;
        private int m_MACHINE_ID = 0;
        private string m_IS_PROCESS_LOSS = string.Empty;

        #endregion  //private members

        #region public events

        public event PropertyChangedEventHandler PropertyChanged;
        private void NotifyPropertyChanged(string info)
        {
            _UpdateChangedList(info);
            if (PropertyChanged != null)
            {
                PropertyChanged(this, new PropertyChangedEventArgs(info));
            }
        }

        #endregion //public events

        #region properties


        [DBColumn(Name = "CONV_BATCH_ID", Storage = "m_CONV_BATCH_ID", DbType = "107", IsDbGenerated = true, SyncOnInsert = true, IsPrimaryKey = true, IsIdentity = true)]
        public int CONV_BATCH_ID
        {
            get { return this.m_CONV_BATCH_ID; }
            set
            {
                this.m_CONV_BATCH_ID = value;
                this.NotifyPropertyChanged("CONV_BATCH_ID");
            }
        }

        [DBColumn(Name = "CONVERSION_ID", Storage = "m_CONVERSION_ID", DbType = "107")]
        public int CONVERSION_ID
        {
            get { return this.m_CONVERSION_ID; }
            set
            {
                this.m_CONVERSION_ID = value;
                this.NotifyPropertyChanged("CONVERSION_ID");
            }
        }

        [DBColumn(Name = "CONVERSION_NO", Storage = "m_CONVERSION_NO", DbType = "126")]
        public string CONVERSION_NO
        {
            get { return this.m_CONVERSION_NO; }
            set
            {
                this.m_CONVERSION_NO = value;
                this.NotifyPropertyChanged("CONVERSION_NO");
            }
        }

        [DBColumn(Name = "ITEM_ID", Storage = "m_ITEM_ID", DbType = "107")]
        public int ITEM_ID
        {
            get { return this.m_ITEM_ID; }
            set
            {
                this.m_ITEM_ID = value;
                this.NotifyPropertyChanged("ITEM_ID");
            }
        }

        [DBColumn(Name = "DEPT_ID", Storage = "m_DEPT_ID", DbType = "107")]
        public int DEPT_ID
        {
            get { return this.m_DEPT_ID; }
            set
            {
                this.m_DEPT_ID = value;
                this.NotifyPropertyChanged("DEPT_ID");
            }
        }

        [DBColumn(Name = "STLM_ID", Storage = "m_STLM_ID", DbType = "107")]
        public int STLM_ID
        {
            get { return this.m_STLM_ID; }
            set
            {
                this.m_STLM_ID = value;
                this.NotifyPropertyChanged("STLM_ID");
            }
        }

        [DBColumn(Name = "PROD_BATCH_NO", Storage = "m_PROD_BATCH_NO", DbType = "126")]
        public string PROD_BATCH_NO
        {
            get { return this.m_PROD_BATCH_NO; }
            set
            {
                this.m_PROD_BATCH_NO = value;
                this.NotifyPropertyChanged("PROD_BATCH_NO");
            }
        }

        [DBColumn(Name = "CLOSING_QTY", Storage = "m_CLOSING_QTY", DbType = "107")]
        public decimal CLOSING_QTY
        {
            get { return this.m_CLOSING_QTY; }
            set
            {
                this.m_CLOSING_QTY = value;
                this.NotifyPropertyChanged("CLOSING_QTY");
            }
        }

        [DBColumn(Name = "UOM_ID", Storage = "m_UOM_ID", DbType = "107")]
        public int UOM_ID
        {
            get { return this.m_UOM_ID; }
            set
            {
                this.m_UOM_ID = value;
                this.NotifyPropertyChanged("UOM_ID");
            }
        }

        [DBColumn(Name = "ITEM_QTY", Storage = "m_ITEM_QTY", DbType = "107")]
        public decimal ITEM_QTY
        {
            get { return this.m_ITEM_QTY; }
            set
            {
                this.m_ITEM_QTY = value;
                this.NotifyPropertyChanged("ITEM_QTY");
            }
        }

        [DBColumn(Name = "SL_NO", Storage = "m_SL_NO", DbType = "107")]
        public int SL_NO
        {
            get { return this.m_SL_NO; }
            set
            {
                this.m_SL_NO = value;
                this.NotifyPropertyChanged("SL_NO");
            }
        }

        [DBColumn(Name = "MACHINE_ID", Storage = "m_MACHINE_ID", DbType = "107")]
        public int MACHINE_ID
        {
            get { return this.m_MACHINE_ID; }
            set
            {
                this.m_MACHINE_ID = value;
                this.NotifyPropertyChanged("MACHINE_ID");
            }
        }

        [DBColumn(Name = "IS_PROCESS_LOSS", Storage = "m_IS_PROCESS_LOSS", DbType = "126")]
        public string IS_PROCESS_LOSS
        {
            get { return this.m_IS_PROCESS_LOSS; }
            set
            {
                this.m_IS_PROCESS_LOSS = value;
                this.NotifyPropertyChanged("IS_PROCESS_LOSS");
            }
        }

        #endregion //properties
    }

    public partial class dcPROD_ITEM_CONVERSION_BATCH
    {
        public int CLOSING_UOM_ID { get; set; }
        public string CLOSING_UOM_NAME { get; set; }
        public decimal USED_QTY { get; set; }
        public decimal SYSTEM_OPENING_STOCK { get; set; }

    }

        
}
