<%@ Page Language="C#" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="GLGroup.aspx.cs" Inherits="PG.Web.Accounting.GeneralLedger.GLGroup" %>
...
// <!CDATA[
		...
		var txtGLGroupNameParent = '<%=txtGLGroupNameParent.ClientID %>';
		var hdnGLGroupIDParent = '<%=hdnGLGroupIDParent.ClientID %>';
		var hdnGLClassID = '<%=hdnGLClassID.ClientID %>';
		var hdnGLGroupClassID = '<%=hdnGLGroupClassID.ClientID %>';
		var hdnGLGroupParentKey = '<%=hdnGLGroupParentKey.ClientID %>';

		var hdnGLGroupParentBalanceType = '<%=hdnGLGroupParentBalanceType.ClientID %>';
		var hdnGLGroupParentIsGrossProfit = '<%=hdnGLGroupParentIsGrossProfit.ClientID %>';
...
$(document).ready(function () {
	...
	$("#" + txtGLGroupNameParent).keydown(function (e) {
		switch (e.keyCode) {
			case 46:  //delete
				ClearData();
				break;
...
function OpenGLGroupTree() {
	...
	if ($("#" + txtGLGroupNameParent).is(":disabled")) {
	...
	if ($("#" + txtGLGroupNameParent).is(":disabled") == false) {
		var glGroupKey = $("#" + hdnGLGroupParentKey).val();             
...
function SetGLGroupData(data) {
	//$("#" + txtGLGroupNameParent).val(data.glgroupname);


	$("#" + txtGLGroupNameParent).val(data.glgroupnameshow);
	$("#" + hdnGLGroupIDParent).val(data.glgroupid);
	$("#" + hdnGLClassID).val(data.glclassid);
	$("#" + hdnGLGroupClassID).val(data.glgroupclassid);
	$("#" + hdnGLGroupParentKey).val(data.glgroupkey);
	$("#" + hdnGLGroupParentBalanceType).val(data.balancetype);
	$("#" + hdnGLGroupParentIsGrossProfit).val(data.isgrossprofit);   
...
function ClearData() {
	if ($('#' + txtGLGroupNameParent).is(":disabled") == false) {
		$("#" + txtGLGroupNameParent).val('');
		$("#" + hdnGLGroupIDParent).val('0');
		$("#" + hdnGLClassID).val('0');
		$("#" + hdnGLGroupClassID).val('0');
		$("#" + hdnGLGroupParentKey).val('');
		$("#" + hdnGLGroupParentBalanceType).val('0');
		$("#" + hdnGLGroupParentIsGrossProfit).val('0');  
		ContentForm.MakeControlIsDirty(txtGLGroupNameParent,true);
	}
}
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
	...
	<div id="dvHeader" class="dvHeader" >
		<asp:Label ID="lblHeader" CssClass="lblHeader" runat="server" Text="GL Head"></asp:Label>
	</div>
	<!--Message Div -->
	...
	<div id = "dvControls" style="height:auto; width:100%">
		<div id="dvControlsInner" class="groupBoxContainer boxShadow">    
			...
			<table cellpadding="2" cellspacing="4">
				 ...
				 <td align="right" >
				   <asp:Label id="Label4" runat="server" Text="GL Group Parent :" ></asp:Label>
					 </td>
				 <td align="left" >
					<div id="dvGLGroup" class="group_linkControl dvGroup" runat="server">
					   <table cellpadding="0" cellspacing="0" border="0">
						   <tr>
							   <td style="">
								   <asp:TextBox ID="txtGLGroupNameParent" runat="server" 
					   ...
					   <input type="hidden" ID="hdnGLGroupIDParent" runat="server"  value="0" />
					   <input type="hidden" ID="hdnGLClassID" runat="server"  value="0" />
					   <input type="hidden" ID="hdnGLGroupClassID" runat="server"  value="0" />
					   <input type="hidden" ID="hdnGLGroupParentKey" runat="server"  value="" />
					   <input type="hidden" ID="hdnGLGroupParentBalanceType" runat="server"  value="0" />
					   <input type="hidden" ID="hdnGLGroupParentIsGrossProfit" runat="server"  value="0" />

					   <input type="hidden" ID="hdnGLGroupIDParentEdit" runat="server"  value="0" />
			...
			<tr>
				 ...
				 <td style="" align="right">
				   <asp:Label id="Label1" runat="server" Text="GL Head Name" ></asp:Label>
				 </td>
				 <td style="" align="left">
				 ...
				 <td align="right" >
				   <asp:Label id="Label3" runat="server" Text="GL Head Code" ></asp:Label>
				 </td>
				 <td align="left" >
