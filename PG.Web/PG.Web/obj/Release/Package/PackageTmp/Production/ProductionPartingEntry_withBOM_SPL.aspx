<%@ Page Title="" Language="C#" EnableEventValidation="false" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="ProductionPartingEntry_withBOM_SPL.aspx.cs" Inherits="PG.Web.Production.ProductionPartingEntry_withBOM_SPL" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        var gridScrapUpdatePanelIDDet = '<%=UpdatePanel4.ClientID%>';
        var gridViewScrapIDDet = '<%=grdScrap.ClientID%>';
        var ScrapupdateProgressID = '<%=UpdateProgress4.ClientID%>';
      ...
      <%--  var txtMachine = '<%= txtMachine.ClientID%>';
            ...
            pageInstance.add_pageLoaded(function (sender, args) {
                ...
                for (i = 0; i < panels.length; i++) {
                    if (panels[i].id == gridUpdatePanelIDDet) {
                        ...
                        bindSupplierList(gridViewIDDet);
                    ...
                    if (panels[i].id == gridScrapUpdatePanelIDDet) {
                        //bindUsedItemInScrapSection(gridViewScrapIDDet);
                        bindScrapItemList(gridViewScrapIDDet);
        ...
        function bindUsedItemInScrapSection(gridViewID) {
            var cgColumns = [
                               { 'columnName': 'itemname', 'width': '200', 'align': 'left', 'highlight': 4, 'label': 'Name' }
                             , { 'columnName': 'uomname', 'width': '70', 'align': 'center', 'highlight': 4, 'label': 'Uom ' }
                             , { 'columnName': 'closing_qty', 'width': '100', 'align': 'center', 'highlight': 4, 'label': 'Closing Qty' }
                             //, { 'columnName': 'bomno', 'width': '160', 'align': 'center', 'highlight': 4, 'label': 'BOM NO' }
            ];
            ...
            $(gridSelector).find('input[id$="txtUsedItemName"]').each(function (index, elem) {

                var elemRow = $(elem).closest('tr.gridRow');

                var hdnItemIDElem = $(elemRow).find('input[id$="txtUsedItemName"]');



                $(elem).closest('tr').find('input[id$="btnUsedItem"]').click(function (e) {
                    elmID = $(elem).attr('id');
                ...
                $(elem).combogrid({
                    ...
                    select: function (event, ui) {
                        ...
                        if (ui.item.id == 0) {
                            ...
                            SetScrapUsedItemData(elemID, ui.item);
                        }
                        // setDetInstrument(hdnIsInstrumentElem, txtInstrumentElem, btnInstrumentElem);
                        return false;
                    }
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        if (prcNo == null) {
                            ...
                            SetScrapUsedItemData(elemID, grp);
                        }
        ...
        function SetScrapUsedItemData(txtItemCodeID, data) {
            $('#' + txtItemCodeID).val(data.itemid);
            var detRow = $('#' + txtItemCodeID).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnUsedItemId"]').val(data.itemid);
            $(detRow).find('input[id$="txtUsedItemName"]').val(data.itemname);
            //$(detRow).find('input[id$="txtClosingQty"]').val(data.closing_qty);
            
        }

        function ClearScrapUsedItemData(txtItemID) {
            var detRow = $('#' + txtItemID).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnUsedItemId"]').val('0');
            $(detRow).find('input[id$="txtUsedItemName"]').val('');
            //$(detRow).find('input[id$="txtClosingQty"]').val('0');
            //sumGrandQty();
        }

        function bindScrapItemList(gridViewIDD) {
            ...
            serviceURL += "&ispaging=1&isFinished=Y&for_production=Y&groupid=48";
            var gridSelector = "#" + gridViewIDD;
            $(gridSelector).find('input[id$="txtScrapItemName"]').each(function (index, elem) {
                ///list click
                var elemRow = $(elem).closest('tr.gridRow');
                var hdnItemIDElem = $(elemRow).find('input[id$="txtScrapItemName"]');
                //var prevGLCode = '';
                $(elem).closest('tr').find('input[id$="btnScrapItem"]').click(function (e) {
                    elmID = $(elem).attr('id');
                    //$(elem).combogrid("show");
                    $(elem).combogrid("dropdownClick");
                });
                ...
                $(elem).combogrid({
                    ...
                    select: function (event, ui) {
                        ...
                        if (ui.item.id == 0) {
                            ...
                            SetScrapItemData(elemID, ui.item);
                        }
                        return false;
                    }
                });
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        var serviceURL = ItemListServiceLink + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
                        serviceURL += "&ispaging=1&isFinished=Y&for_production=Y&groupid=48" + "&deptid=" + DeptID + "&stlmid=" + stlmid;
                        var prcNo = GetItemNo(eCode, serviceURL);
                        if (prcNo == null) {
                            ClearScrapItemData(elemID);
                        }
                        else {
                            SetScrapItemData(elemID, grp);
                        }
                    }
                });
        ...
        function ClearScrapItemData(txtCLOSINGITEM_NAME) {
            //$('#' + txtGLAccCodeID).val('');
            var detRow = $('#' + txtCLOSINGITEM_NAME).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnScrapItemId"]').val('0');
            $(detRow).find('input[id$="txtScrapItemName"]').val('');
            $(detRow).find('input[id$="txtScrapItemUOM"]').val('');
            $(detRow).find('input[id$="hdnScrapItemUOMId"]').val('0');
        
        }

        function SetScrapItemData(txtCLOSINGITEM_NAME, data) {
            var detRow = $('#' + txtCLOSINGITEM_NAME).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnScrapItemId"]').val(data.itemid);
            $(detRow).find('input[id$="txtScrapItemName"]').val(data.itemname);
            $(detRow).find('input[id$="txtScrapItemUOM"]').val(data.uomname);
            $(detRow).find('input[id$="hdnScrapItemUOMId"]').val(data.uomid);
        ...
        function ClearItemData(txtItemID) {
            ...
            //$(detRow).find('input[id$="txtFalseLugName"]').val('');
            //$(detRow).find('input[id$="hdnFalseLugID"]').val('');
        ...
        function SetItemData(txtItemCodeID, data) {
            ...
            //$(detRow).find('input[id$="txtFalseLugName"]').val(data.falselug_itemName);
            //$(detRow).find('input[id$="hdnFalseLugID"]').val(data.falselug_itemid);
        ...
        function TestOnTextChange(texbx) {
            ...
            var RejectQty = $(detRow).find('input[id$="txtRejectQty"]').val();
            var panelqty = $(detRow).find('input[id$="txtItem_qty"]').val();

            var tpcqty = panelqty;
            $(detRow).find('input[id$="txtItem_qty"]').val(tpcqty);
            var TotQty = parseFloat(RejectQty) + parseFloat(tpcqty);
            var standardweight = $(detRow).find('input[id$="hdnITEM_STANDARD_WEIGHT_KG"]').val();
            var tweight = tpcqty * standardweight;
            $(detRow).find('input[id$="txtITEM_WEIGHT"]').val(tweight);
            $('#' + hdnFinishedQty).val(TotQty);
                ...
                $(elem).combogrid({
                    ...
                    search: function (event, ui) {
                        ...
                        var e = document.getElementById("<%=ddlDEPT_ID.ClientID%>");
                         ...
                         var newServiceURL = serviceURL + "&machinename=" + machinename + "&deptid=" + dept_id + "&stlid=" + stlid; //" + dept_id + "
                         newServiceURL = JSUtility.AddTimeToQueryString(newServiceURL);
                         $(this).combogrid("option", "url", newServiceURL);
       ...
       <%-- function bindMachineMstList() {
         ...
         function bindSupplierList(gridViewID) {
             var cgColumns = [{ 'columnName': 'supcode', 'width': '80', 'align': 'left', 'highlight': 4, 'label': 'Sup Code' }
                              , { 'columnName': 'supname', 'width': '300', 'align': 'left', 'highlight': 4, 'label': 'Sup Name' }
             ...
             var gridSelector = "#" + gridViewID;
             $(gridSelector).find('input[id$="txtSupName"]').each(function (index, elem) {
                 ///list click
                 var elemRow = $(elem).closest('tr.gridRow');

                 var hdnItemIDElem = $(elemRow).find('input[id$="txtSupName"]');

                 //var prevGLCode = '';

                 $(elem).closest('tr').find('input[id$="btnSupId"]').click(function (e) {
                     elmID = $(elem).attr('id');
                     //$(elem).combogrid("show");
                     $(elem).combogrid("dropdownClick");
                 });
                 ...
                 $(elem).combogrid({
                     ...
                     search: function (event, ui) {
                        ...
                        // var name = $(elemRow).find('input[id$="txtOPERATOR_NAME"]').val();
                         //" + dept_id + "
                         var newServiceURL = serviceURL;// + "&name=" + name + "&deptid=" + dept_id;
                         newServiceURL = JSUtility.AddTimeToQueryString(newServiceURL);
                         $(this).combogrid("option", "url", newServiceURL);
                     ...
                     select: function (event, ui) {
                         ...
                         if (ui.item.id == 0) {
                             ...
                             SetSupplierData(elemID, ui.item);
                         }
                         // setDetInstrument(hdnIsInstrumentElem, txtInstrumentElem, btnInstrumentElem);
                         return false;
                     }
                 ...
                 $(elem).blur(function () {
                     ...
                     //setDetInstrument(hdnIsInstrumentElem, txtInstrumentElem, btnInstrumentElem);
                     grpID = $(self).closest('tr').find('input[id$="hdnSupId"]').val();
                     if (grpID == '0' | grpID == '') {
                         $(self).addClass('textError');
                     }
        ...
        function SetSupplierData(txtSupName, data) {
            $('#' + txtSupName).val(data.supname);
            var detRow = $('#' + txtSupName).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnSupId"]').val(data.supid);

         }
        function ClearSupplierData(txtSupName) {
            var detRow = $('#' + txtSupName).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnSupId"]').val('0');
             $(detRow).find('input[id$="txtSupName"]').val('');
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="dvPageContent" style="width: 100%; height: 100%;" onkeydown="if(event.keyCode==13){event.keyCode=9; return event.keyCode;}">
        ...
        <asp:HiddenField ID="hdnStoragelocationID" runat="server" Value="10" />
        <div id="dvContentHeader" class="dvContentHeader">
            <div id="dvHeader" class="dvHeader_Prod" align="left">
                <asp:Label ID="lblHeader" runat="server" Text="Production Parting Entry" CssClass="lblHeader" Font-Bold="true" Font-Size="15px"></asp:Label>
            </div>
 ...
 <div id="tabs" style="height: 95%; background : url(../image/bg_greendot.gif) !important;font-size:12px;">
  <ul>
    ...
    <li><a href="#tabs-3">Scrap Details </a></li>
                    ...
                    <tr>
                       ...
                       <td style="text-align: right" class="auto-style1">
                        ...
                        <td style="text-align: left" colspan="" class="auto-style1">
                            <asp:TextBox ID="txtProdRemarks" runat="server" CssClass="textBox textAutoSelect" Width="300px"  TabIndex="5"  ></asp:TextBox>
                        ...
                        <td>
                             <asp:TextBox ID="TextBox1" runat="server" CssClass="textBox" Style="text-align: left;background-color:gray; color : white;font-size:larger; font-weight: 700;" Text="Parting" Width="150px" ReadOnly="true"></asp:TextBox>
                        </td>
 ...
 <div id="tabs-1">
            ...
            <div id="groupBoxContainer boxShadow" style="height: auto; width: 100%;">
                 <div id="dvGridHeader2" style="float: left; width: 1224px; height: 30px; font-size: smaller;" class="subHeader_Prod">
                <table style="height: 80%; color: #FFFFFF; font-weight: bold; text-align: left;"
                    ...
                    <tr class="headerRow_Prod">
                        <td width="30px" class="headerColCenter_prod">SL#
                         ...
                         <td width="120px" class="headerColCenter_prod">Machine Name
                        </td>
                        <td width="220px" class="headerColCenter_prod">FG Item
                        </td>
                        <td width="15px" class="headerColCenter_prod"></td>

                         <td width="65px" class="headerColCenter_prod">Reject Qty
                        </td>
                        
                        <td width="65px" class="headerColCenter_prod">Item Qty
                        </td>
                      ...
                      <%--  <td width="100px" class="headerColCenter"> False Lug
                        </td>--%>
                ...
                <div id="groupDataDetails" style="width: 100%; height: auto;">
                    ...
                    <div id="dvGridContainer2" class=" " runat="server" style="height: auto; text-align: left; width: 100%">
                        ...
                        <div id="dvGrid" style="width: 1224px; height: 200px; overflow: auto;" class="dvGrid">
                                                        ...
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtMACHINE_NAME" runat="server" CssClass="textBox textAutoSelect" Width="105px" style="text-align:left; font-size : 10px;" Text='<%#Bind("MACHINE_NAME") %>'></asp:TextBox>
                                                                    ...
                                                                    <td>
                                                                     ...
                                                                     <td align="right">
                                                                        <asp:TextBox ID="txtRejectQty" runat="server" CssClass="txtPcQty textBox textAutoSelect" Width="65px" align="right" Style="text-align: right;" Text='<%# Bind("REJECT_QTY") %>' onblur="TestOnTextChange(this)"  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>

                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtItem_qty" runat="server" CssClass="txtPcQty textBox textAutoSelect" Width="65px" align="right" Style="text-align: right;" Text='<%# Bind("ITEM_QTY") %>' onblur="TestOnTextChange(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <asp:TextBox ID="txtITEM_STANDARD_WEIGHT_KG" runat="server" CssClass="txtstdweightQty textBox textAutoSelect" Width="45px" style="text-align:right;font-size : 11px;" Text='<%# Bind("ITEM_STANDARD_WEIGHT_KG") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>

                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtITEM_WEIGHT" runat="server" CssClass="txtWeightQty textBox textAutoSelect" Width="50px" align="left" Text='<%# Eval("ITEM_WEIGHT", "{0:#,##0.00}") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td>
                                                                        <asp:TextBox ID="txtWEIGHT_UOM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="40px" Text='<%# Bind("WEIGHT_UOM_NAME") %>' Style=""></asp:TextBox>
                                                                  ...
                                                                  <%--     <td>
                                                                        <asp:TextBox ID="txtSupName" runat="server" CssClass="textBox textAutoSelect" Width="100px" Text='<%#Bind("SUP_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnSupId" runat="server" Value='<%# Bind("RM_SUP_ID") %>' />
                                                                    </td>--%>
                                                                  <%--  <td>
                                                                        <input id="btnSupId" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>--%>
                                                                  ...
                                                                  <%--    <td>
                                                                        <asp:TextBox ID="txtFalseLugName" runat="server" CssClass="textBox textAutoSelect" Width="100px" Text='<%#Bind("FALSE_LUG_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnFalseLugID" runat="server" Value='<%# Bind("FALSE_LUG_ITEM_ID") %>' />
                                                                       </td>--%>
                                                                 ...
                                                                 <td>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <asp:TextBox ID="txtBatchNoDtl" runat="server" CssClass="textBox textAutoSelect" Width="100px" Text='<%# Bind("PROD_BATCH_NO_DTL") %>' Style="display:none"></asp:TextBox>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                        ...
                        <div id="divGridControls2" style="width: 100%; height: 30px; border-top: solid 1px #C0C0C0; border-bottom: solid 1px #0b07f5;">
                            <table style="width: 70%; height: 100%; text-align: center;" cellspacing="1" cellpadding="1"
                                ...
                                <tr>
                                    ...
                                    <td align="right" style="width: 140px; text-align: right;">
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                  ...
                                                                  <td> 
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtSYSTEM_OPENING_STOCK" runat="server" CssClass="textBox textAutoSelect"  Width="112px" style="text-align:right;" Text='<%# Eval("SYSTEM_OPENING_STOCK", "{0:#,##0.00}") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtCLOSING_QTY" runat="server" CssClass="textBox textAutoSelect" Width="75" style="text-align:right;" Text='<%# Eval("CLOSING_QTY", "{0:#,##0.00}") %>' onchange="calcIssueStock(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                   ...
                                                                   <%-- <td align="right">
                                                                    ...
                                                                    <%--  <td>
                                                                        ...
                                                                        <td align="right">
                                                                        <asp:TextBox ID="txtSTD_USED_QTY" runat="server" CssClass="textBox textAutoSelect" Width="77"  align="right" style="text-align:right;" Text='<%# Eval( "STD_USED_QTY", "{0:#,##0.00}") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtPOSITIVE_DEV" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="62px" style="text-align:right;" BackColor="Yellow"  Text='<%# Eval( "POSITIVE_DEV", "{0:#,##0.00}") %>'  onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
 ...
 <div id="tabs-3">
                    ...
                    <div id="Div2" runat="server" class="" style="float:left; width: 100%; text-align: left;  border-top: solid 1px #0b07f5;">
                        <span style="font-weight: bold; font-size: 15; color: #ff3b00;">Scrap Details: </span>
                        <asp:Label runat="server" ID="lblScrapNo"></asp:Label>
                    </div>
           <div id="groupBoxScrap" style="height: auto; ">
                <div id="groupDataScrapDetails" style="width: 100%; height: auto;">
                    <div id="Div4" runat="server" class="" style="width: auto; height: auto; text-align: left; width: 100%">
                         <div id="dvGridHeaderScrap" style="float:left; width: 1000px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                class="defFont" cellspacing="1" cellpadding="1">
                                <tr class="headerRow_Prod">
                                    <td width="40px" class="headerColCenter_prod">SL#
                                    </td>
                                   <%-- <td width="180px" class="headerColCenter_prod">Used Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>--%>
                                    <td width="200px" class="headerColCenter_prod"> Scrap Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>
                                    ...
                                    <td width="65px" class="headerColCenter_prod">UOM
                                     ...
                                     <td width="75px" class="headerColCenter_prod"> Qty
                                    </td>
                                  <%--   <td width="75px" class="headerColCenter_prod">Closing Qty
                                    </td>--%>
                                    <td width="110px" class="headerColCenter_prod">Remarks
                        ...
                        <div id="dvGridScrap" style=" float:left; width: 1000px; height: 200px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel4" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grdScrap" runat="server" width="1000px" AutoGenerateColumns="False" ShowHeader="False"
                                        CellPadding="1" CellSpacing="1" ForeColor="#333333" GridLines="Both" DataKeyNames="ITEM_ID"
                                        EnableModelValidation="True" ClientIDMode="AutoID" OnRowCommand="grdScrap_RowCommand" OnRowCreated="grdScrap_RowCreated" OnRowDataBound="grdScrap_RowDataBound" OnRowDeleting="grdScrap_RowDeleting">
                                        <RowStyle BackColor="#F7F6F3" ForeColor="#333333" />
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderText="SL#" >
                                                <ItemTemplate>
                                                    <asp:Label ID="lblScrapSl" runat="server" Text= <%# Container.DataItemIndex + 1 %>   Style="text-align: center;" Width="40px">
                                                    </asp:Label>
                                                    ...
                                                    <div>
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnScrap_MST_ID" runat="server" Value='<%# Bind("SCRAP_ID") %>' />
                                                                         <asp:HiddenField ID="hdnScrapDtlId" runat="server" Value='<%# Bind("SCRAP_DTL_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_MST_ID" runat="server" Value='<%# Bind("PROD_ID") %>' />
                                                                       <%-- <asp:HiddenField ID="hdnISMANUAL" runat="server" Value='<%# Bind("ISMANUAL") %>' />--%>
                                                                    </td>

                                                               <%--     <td>
                                                                        <asp:TextBox ID="txtUsedItemName" runat="server" CssClass="textBox textAutoSelect" Width="175px" Text='<%# Bind("USED_ITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnUsedItemId" runat="server" Value='<%# Bind("USED_ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnUsedItem" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>--%>
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtScrapItemName" runat="server" CssClass="textBox textAutoSelect" Width="198px" Text='<%# Bind("ITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnScrapItemId" runat="server" Value='<%# Bind("ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnScrapItem" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>
                                                                    <td>

                                                                        <asp:TextBox ID="txtScrapItemUOM" runat="server" CssClass="textBox textAutoSelect" Width="62px" Text='<%# Bind("UOM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnScrapItemUOMId" runat="server" Value='<%# Bind("UOM_ID") %>' />
                                                                    </td>
                              

                                                               
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtScrapQty" runat="server" CssClass="textBox textAutoSelect" Width="72"  align="right" style="text-align:right;"  Text='<%# Eval("ITEM_QTY") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <%--   <td align="right">
                                                                        <asp:TextBox ID="txtClosingQty" runat="server" CssClass="textBox textAutoSelect" Width="72"  align="right" style="text-align:right;"  Text='<%# Eval("CLOSING_QTY") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>--%>
                                                                    <td>
                                                                        <asp:TextBox ID="txtScrapRemarks" runat="server" CssClass="textBox textAutoSelect" Width="110px" Text='<%# Bind("REMARKS") %>' Style=""></asp:TextBox>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                ...
                                <Triggers>
                                    <asp:AsyncPostBackTrigger ControlID="btnNewRowScrap" EventName="Click" />
                                    <%-- <asp:AsyncPostBackTrigger ControlID="btnRefreshClosingGrid" EventName="Click" />--%>
                                </Triggers>
                            </asp:UpdatePanel>
                        </div>

                        <div id="divGridControlsScrap">
                            <table style="width: 70%; height: 100%; text-align: center;" cellspacing="1" cellpadding="1"
                                border="0">
                                <tr>
                                    <td style="width: 2px"></td>
                                    ...
                                    <td style="width: 90px" align="left">
                                       <asp:Button ID="btnNewRowScrap" runat="server"  Text="" CssClass="buttonNewRow" OnClientClick="ShowClosingProgress()" OnClick="btnNewRowScrap_Click" Enabled="true" />
                                    </td>
                                    <td style="width: 20px;">
                                        <asp:UpdateProgress ID="UpdateProgress4" runat="server" AssociatedUpdatePanelID="UpdatePanel4"
                                            DisplayAfter="300">
                                            <ProgressTemplate>
                                                <asp:Image ID="imgProgressScrap" runat="server" ImageUrl="~/image/loader.gif" />
                                            </ProgressTemplate>
                                        </asp:UpdateProgress>
                                    </td>
                                </tr>
