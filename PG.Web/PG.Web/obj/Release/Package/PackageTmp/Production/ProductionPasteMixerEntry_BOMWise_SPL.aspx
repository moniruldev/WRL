<%@ Page Title="" Language="C#" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="ProductionPasteMixerEntry_BOMWise_SPL.aspx.cs" Inherits="PG.Web.Production.ProductionPasteMixerEntry_BOMWise_SPL" %>
...
<%--        var gridDrossUpdatePanelIDDet = '<%=UpdatePanel4.ClientID%>';
        var gridViewDrossIDDet = '<%=grdDross.ClientID%>';
        var DrossupdateProgressID = '<%=UpdateProgress4.ClientID%>';--%>
        ...
        function CalculateTotalQty() {
            ...
            for (var i = 0; i < grid.rows.length; i++) {
                sumUsed += parseFloat($("input[id*=txtItemWeightQty]")[i].value);
        ...
        $(document).ready(function () {
            ...
            pageInstance.add_pageLoaded(function (sender, args) {
                ...
                for (i = 0; i < panels.length; i++) {
                    ...
                    //if (panels[i].id == gridDrossUpdatePanelIDDet) {
                    //    bindUsedItemInDrossSection(gridViewDrossIDDet);
                    //    bindDrossItemList(gridViewDrossIDDet);
        ...
        function bindItemList(gridViewID) {
            var cgColumns = [
                             ...
                             , { 'columnName': 'bomname', 'width': '200', 'align': 'center', 'highlight': 4, 'label': 'BOM Name' }
                             , { 'columnName': 'bomno', 'width': '200', 'align': 'center', 'highlight': 4, 'label': 'BOM NO' }
            ];
        ...
        function bindUsedItemInDrossSection(gridViewID) {
            var cgColumns = [
                               { 'columnName': 'itemname', 'width': '200', 'align': 'left', 'highlight': 4, 'label': 'Name' }
                             , { 'columnName': 'uomname', 'width': '70', 'align': 'center', 'highlight': 4, 'label': 'Uom ' }
                             , { 'columnName': 'bomname', 'width': '150', 'align': 'center', 'highlight': 4, 'label': 'BOM Name' }
            ...
            var serviceURL = BOMNameListServiceLink + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            serviceURL += "&ispaging=1&isFinished=N&deptid=143&for_production=Y";
                ...
                $(elem).combogrid({
                    ...
                    select: function (event, ui) {
                        ...
                        if (ui.item.id == 0) {
                            ...
                            SetDrossUsedItemData(elemID, ui.item);
                        }
                        // setDetInstrument(hdnIsInstrumentElem, txtInstrumentElem, btnInstrumentElem);
                        return false;
                    }
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        if (prcNo == null) {
                            ...
                            SetDrossUsedItemData(elemID, grp);
                        }
        ...
        function GetItemNo(eCode, serviceURL) {
            ...
            var newServiceURL = serviceURL + "&deptid=143&selectedId=" + eCode;
            var dummyVar = $.ajax({
                type: "GET",
                cache: false,
                async: false,
        ...
        function SetDrossUsedItemData(txtItemCodeID, data) {
        ...
        function ClearDrossUsedItemData(txtItemID) {
        ...
        function bindDrossItemList(gridViewIDD) {
            ...
            serviceURL += "&ispaging=1&deptid=143&isFinished=Y&for_production=Y&groupid=47";
            var gridSelector = "#" + gridViewIDD;
            $(gridSelector).find('input[id$="txtDrossItemName"]').each(function (index, elem) {
                ///list click
                var elemRow = $(elem).closest('tr.gridRow');
                var hdnItemIDElem = $(elemRow).find('input[id$="txtDrossItemName"]');
                //var prevGLCode = '';
                $(elem).closest('tr').find('input[id$="btnDrossItem"]').click(function (e) {
                    elmID = $(elem).attr('id');
                    //$(elem).combogrid("show");
                    $(elem).combogrid("dropdownClick");
                });
        ...
        function ClearDrossItemData(txtCLOSINGITEM_NAME) {
            ...
            $(detRow).find('input[id$="hdnDrossItemId"]').val('0');
            $(detRow).find('input[id$="txtDrossItemName"]').val('');
            $(detRow).find('input[id$="txtDrossItemUOM"]').val('');
            $(detRow).find('input[id$="hdnDossItemUOMId"]').val('0');

        }

        function SetDrossItemData(txtCLOSINGITEM_NAME, data) {
            var detRow = $('#' + txtCLOSINGITEM_NAME).closest('tr.gridRow');
            $(detRow).find('input[id$="hdnDrossItemId"]').val(data.itemid);
            $(detRow).find('input[id$="txtDrossItemName"]').val(data.itemname);
            $(detRow).find('input[id$="txtDrossItemUOM"]').val(data.uomname);
            $(detRow).find('input[id$="hdnDossItemUOMId"]').val(data.uomid);
            ...
            $(gridSelector).find('input[id$="txtMACHINE_NAME"]').each(function (index, elem) {
                ...
                $(elem).combogrid({
                    ...
                    search: function (event, ui) {
                        ...
                        var newServiceURL = serviceURL + "&machinename=" + machinename + "&deptid=" + DeptID +"&stlid=" + stlid; //" + dept_id + "
                        newServiceURL = JSUtility.AddTimeToQueryString(newServiceURL);
                        $(this).combogrid("option", "url", newServiceURL);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="dvPageContent" style="width: 100%; height: 100%;" onkeydown="if(event.keyCode==13){event.keyCode=9; return event.keyCode;}">
        ...
        <asp:HiddenField ID="hdnStoragelocationID" runat="server" Value="5" />
        <div id="dvContentHeader" class="dvContentHeader">
            <div id="dvHeader" class="dvHeader_Prod" align="left">
                <asp:Label ID="lblHeader" runat="server" Text="Production Paste Mixer Entry" CssClass="lblHeader" Font-Bold="true" Font-Size="15px"></asp:Label>
            </div>
 ...
 <div id="tabs" style="height: 95%; background : url(../image/bg_greendot.gif) !important;font-size:12px;">
  <ul>
   ...
   <%-- <li><a href="#tabs-3">Dross Details </a></li>--%>
               ...
               <table border="0" cellspacing="4" cellpadding="2" align="left" style="width: 90%;font-size:12px;margin:5px;" id="tblProductionMstEntry">
                    ...
                    <tr>
                       ...
                       <td style="text-align: right" class="auto-style1">
                        ...
                        <td>
                             <asp:TextBox ID="txtStorageLocation" runat="server" CssClass="textBox" Style="text-align: left;background-color:gray; color : white;font-size:larger; font-weight: 700;" Text="Paste Mixer" Width="200px" ReadOnly="true"></asp:TextBox>
                        </td>
 ...
 <div id="tabs-1">
            ...
            <div id="groupBoxContainer boxShadow" style="height: auto; width: 100%;">
                 <div id="dvGridHeader2" style="float: left; width: 1220px; height: 30px; font-size: smaller;" class="subHeader_Prod">
                <table style="height: 80%; color: #FFFFFF; font-weight: bold; text-align: left;"
                    ...
                    <tr class="headerRow_Prod">
                        ...
                        <td width="230px" class="headerColCenter_prod">FG Item
                        </td>
                        <td width="15px" class="headerColCenter_prod"></td>
                        
                        <td width="61px" class="headerColCenter_prod">Batch Qty
                        </td>
                         <td width="80px" class="headerColCenter_prod">Item Qty
                        </td>
                        <td width="43px" class="headerColCenter_prod">UOM
                        </td>
                        <td width="30px" class="headerColCenter_prod">STD Weight
                        </td>
                        <td width="50px" class="headerColCenter_prod">Item Weight
                        </td>
                        <td width="45px" class="headerColCenter_prod">Weight UOM
                        </td>

                        <td width="112px" class="headerColCenter_prod">Operator
                        </td>
                ...
                <div id="groupDataDetails" style="width: 100%; height: auto;">
                    ...
                    <div id="dvGridContainer2" class=" " runat="server" style="height: auto; text-align: left; width: 100%">
                        ...
                        <div id="dvGrid" style="width: 1220px; height: 200px; overflow: auto;" class="dvGrid">
                                                        ...
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="230px" Text='<%# Bind("ITEM_NAME") %>' Style="font-size:10px;font-weight:bold;color:black;"></asp:TextBox>
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtItem_qty" runat="server" CssClass="txtPcQty textBox textAutoSelect" Width="60px" align="right" Style="text-align: right;" Text='<%# Bind("MIXER_BATCH_QTY") %>' onblur="TestOnTextChange(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                     <td align="right">
                                                                        <asp:TextBox ID="txtItemWeightQty" runat="server" CssClass="txtPcQty textBox textAutoSelect" Width="80px" align="right" Style="text-align: right;" Text='<%# Bind("ITEM_QTY") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                  
                                                                    <%--onchange="TestOnTextChange(this)"--%>
                                                                    <td>

                                                                        <asp:TextBox ID="txtUOM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="43px" Text='<%# Bind("UOM_NAME") %>'></asp:TextBox>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <asp:TextBox ID="txtITEM_STANDARD_WEIGHT_KG" runat="server" CssClass="txtstdweightQty textBox textAutoSelect" Width="42px" style="text-align:right;" Text='<%# Bind("ITEM_STANDARD_WEIGHT_KG") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
 ...
 <div id="tabs-2">
                        ...
                        <div id="dvGridHeaderClosing" style="float:left; width: 1200px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                ...
                                <tr class="headerRow_Prod">
                                    ...
                                    <td width="200px" class="headerColCenter_prod">Finished Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>
                                   ...
                                   <%-- <td width="75px" class="headerColCenter_prod">Reuse Qty
                                    ...
                                    <td width="52px" class="headerColCenter_prod"> Pos Dev
                                    </td>
                                    <td width="52px" class="headerColCenter_prod"> Neg Dev
                                    </td>
                                                        ...
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtFINISH_ITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="200px" Text='<%# Bind("FINISH_ITEM_NAME") %>'></asp:TextBox>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <asp:TextBox ID="txtCLOSING_UOM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="50px" Text='<%# Bind("CLOSING_UOM_NAME") %>'></asp:TextBox>
                                                                   ...
                                                                   <td> 
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtSYSTEM_OPENING_STOCK" runat="server" CssClass="textBox textAutoSelect"  Width="78px" style="text-align:right;" Text='<%# Eval("SYSTEM_OPENING_STOCK", "{0:#,##0.00}") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                   ...
                                                                   <%-- <td align="right">
                                                                    ...
                                                                    <%--  <td>
                                                                        ...
                                                                        <td align="right">
                                                                        <asp:TextBox ID="txtSTD_USED_QTY" runat="server" CssClass="textBox textAutoSelect" Width="75"  align="right" style="text-align:right;" Text='<%# Eval( "STD_USED_QTY", "{0:#,##0.00}") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtPOSITIVE_DEV" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="52px" style="text-align:right;" BackColor="Yellow"  Text='<%# Eval( "POSITIVE_DEV", "{0:#,##0.00}") %>'  onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtNEGATIVE_DEV" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="50" style="text-align:right;" BackColor="Yellow"  Text='<%# Eval("NEGATIVE_DEV", "{0:#,##0.00}") %>' onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox> 
                                                                    </td>
                                                                    <td align="right">
    ...
    <%-- <div id="tabs-3">
           ...
           <div id="groupBoxDross" style="height: auto; ">
                <div id="groupDataDrossDetails" style="width: 100%; height: auto;">
                    <div id="Div4" runat="server" class="" style="width: auto; height: auto; text-align: left; width: 100%">
                         <div id="dvGridHeaderDross" style="float:left; width: 1000px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                ...
                                <tr class="headerRow_Prod">
                                    ...
                                    <td width="15px" class="headerColCenter_prod"></td>
                                    <td width="200px" class="headerColCenter_prod"> Dross Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>
                        ...
                        <div id="dvGridDross" style=" float:left; width: 1000px; height: 200px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel4" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grdDross" runat="server" width="1000px" AutoGenerateColumns="False" ShowHeader="False"
                                        CellPadding="1" CellSpacing="1" ForeColor="#333333" GridLines="Both" DataKeyNames="ITEM_ID"
                                        EnableModelValidation="True" ClientIDMode="AutoID" OnRowCommand="grdDross_RowCommand" OnRowCreated="grdDross_RowCreated" OnRowDataBound="grdDross_RowDataBound" OnRowDeleting="grdDross_RowDeleting">
                                        <RowStyle BackColor="#F7F6F3" ForeColor="#333333" />
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderText="SL#" >
                                                <ItemTemplate>
                                                    <asp:Label ID="lblDrossSl" runat="server" Text= <%# Container.DataItemIndex + 1 %>   Style="text-align: center;" Width="40px">
                                                    </asp:Label>
                                                    ...
                                                    <div>
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnDROSS_MST_ID" runat="server" Value='<%# Bind("DROSS_MST_ID") %>' />
                                                                         <asp:HiddenField ID="hdnDrossDtlId" runat="server" Value='<%# Bind("DROSS_DTL_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_MST_ID" runat="server" Value='<%# Bind("PROD_MST_ID") %>' />
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtDrossItemName" runat="server" CssClass="textBox textAutoSelect" Width="198px" Text='<%# Bind("ITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnDrossItemId" runat="server" Value='<%# Bind("ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnDrossItem" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>
                                                                    <td>

                                                                        <asp:TextBox ID="txtDrossItemUOM" runat="server" CssClass="textBox textAutoSelect" Width="62px" Text='<%# Bind("UOM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnDossItemUOMId" runat="server" Value='<%# Bind("UOM_ID") %>' />
                                                                    </td>
                              

                                                               
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtDrossQty" runat="server" CssClass="textBox textAutoSelect" Width="72"  align="right" style="text-align:right;"  Text='<%# Eval("ITEM_QTY") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td>
                                                                        <asp:TextBox ID="txtDrossRemarks" runat="server" CssClass="textBox textAutoSelect" Width="110px" Text='<%# Bind("REMARKS") %>' Style=""></asp:TextBox>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                ...
                                <Triggers>
                                    <asp:AsyncPostBackTrigger ControlID="btnNewRowDross" EventName="Click" />
                        ...
                        <div id="divGridControlsDross">
                            <table style="width: 70%; height: 100%; text-align: center;" cellspacing="1" cellpadding="1"
                                ...
                                <tr>
                                    ...
                                    <td style="width: 90px" align="left">
                                       <asp:Button ID="btnNewRowDross" runat="server"  Text="" CssClass="buttonNewRow" OnClientClick="ShowClosingProgress()" OnClick="btnNewRowDross_Click" Enabled="false" />
                                    </td>
                                    <td style="width: 20px;">
                                        <asp:UpdateProgress ID="UpdateProgress4" runat="server" AssociatedUpdatePanelID="UpdatePanel4"
                                            DisplayAfter="300">
                                            <ProgressTemplate>
                                                <asp:Image ID="imgProgressDross" runat="server" ImageUrl="~/image/loader.gif" />
                                            </ProgressTemplate>
                                        </asp:UpdateProgress>
                                    </td>
                                </tr>
