<%@ Page Title="" Language="C#" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="Production_Asm_Plate_Rejection_Declare.aspx.cs" Inherits="PG.Web.Production.Production_Asm_Plate_Rejection_Declare" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        txtRejectionDate = '<%=txtRejectionDate.ClientID%>';
        ...
        function isDateValidKey(evt, obj) {
        ...
        function calcWeight(texbx) {
            ...
            $(detRow).find('input[id$="txtREJECTION_WEIGHT"]').val(Number(tweight).toFixed(2));
        ...
        function bindItemList(gridViewID) {
            ...
            //var DEPT_ID = $('#' + hdnFromDepartmentID).val();
            //alert(DEPT_ID);
            //var serviceURL = GLAccountServiceLink + "?isterm=1&includeempty=1&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            var serviceURL = ItemListServiceLinkd + "?isterm=1&includeempty=0&iscodename=1&ispaging=1&isFinished=Y&rejectType=Plate&codecomptype=" + Enums.DataCompareType.StartsWith;
            ...
            $(gridSelector).find('input[id$="txtITEM_NAME"]').each(function (index, elem) {
                ...
                $(elem).combogrid({
                    ...
                    search: function (event, ui) {
                     ...
                     //   var vbatchNo = $(elemRowCur).find('input[id$="txtBatchNo"]').val();
                        var deptid = $(elemRowCur).find('input[id$="hdnFromDepartmentID"]').val();
...
//"&batchno=" + vbatchNo +
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        serviceURL += "&ispaging=1&deptid=" + deptid;
        ...
        function SetItemData(txtItemCodeID, data) {
            ...
            $(detRow).find('input[id$="txtITEM_STANDARD_WEIGHT_KG"]').val(Number(data.itemstandardweightkg).toFixed(4));
           ...
           // alert($(detRow).find('input[id$="txtRejection_QTY"]').val());
            if ($(detRow).find('input[id$="txtRejection_QTY"]').val() > 0)
            {
                //alert(data.itemstandardweightkg * $(detRow).find('input[id$="txtRejection_QTY"]').val());
                $(detRow).find('input[id$="txtREJECTION_WEIGHT"]').val(Number(data.itemstandardweightkg * $(detRow).find('input[id$="txtRejection_QTY"]').val()).toFixed(2));
            }
        ...
        function bindBatchList(gridViewID) {
            ...
            serviceURL += "&ispaging=1&deptid=0";
            var gridSelector = "#" + gridViewID;
            $(gridSelector).find('input[id$="txtBatchNo"]').each(function (index, elem) {
                ...
                $(elem).combogrid({
                    ...
                    search: function (event, ui) {
                        var elemRowCur = $(elem).closest('tr.gridRow');
                        var DEPT_ID = $(elemRowCur).find('input[id$="hdnFromDepartmentID"]').val();
                        //var proddate = $('#' + txtProductionDate).val();
                        var serviceURL = ProductionDateWiseBatch_List + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
        ...
        function bindDeptList(gridViewID) {
            var cgColumns = [
                               { 'columnName': 'deptname', 'width': '150', 'align': 'left', 'highlight': 4, 'label': 'Dept Name' }
                               , { 'columnName': 'deptcode', 'width': '100', 'align': 'left', 'highlight': 4, 'label': 'Dept Code' }
                               
            ];
           // var DEPT_ID = $('#' + ddlDEPT_ID).val();
           // var proddate = $('#' + txtProductionDate).val();
            var serviceURL = DeptListLinkd + "?isterm=1&includeempty=0&iscodename=1&ispaging=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            //serviceURL += "&ispaging=1&isFinished=Y&deptid=" + DEPT_ID + "&proddate=" + proddate;
            var gridSelector = "#" + gridViewID;
            $(gridSelector).find('input[id$="txtDeptName"]').each(function (index, elem) {
                ...
                $(elem).closest('tr').find('input[id$="btnDeptID"]').click(function (e) {
                    elmID = $(elem).attr('id');
                    $(elem).combogrid("dropdownClick");
                });
                $(elem).combogrid({
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        //var serviceURL = DeptListLinkd + "?isterm=1&includeempty=0&iscodename=0&codecomptype=" + Enums.DataCompareType.StartsWith;
                        //serviceURL += "&ispaging=1&";
        ...
        function ClearDeptData(txtDept) {
            //$('#' + txtGLAccCodeID).val('');
            $(detRow).find('input[id$="txtDeptName"]').val('');
            var detRow = $('#' + txtDept).closest('tr.gridRow');
            $(detRow).find('input[id$="txtDeptName"]').val('');

        }
        function SetDeptData(txtDept, data) {
            $('#' + txtDept).val(data.itemid);

            var detRow = $('#' + txtDept).closest('tr.gridRow');

            $(detRow).find('input[id$="txtDeptName"]').val(data.deptname);
            $(detRow).find('input[id$="hdnFromDepartmentID"]').val(data.deptid);
        ...
        //function ClearDeptData(txtItemID) {
        //    //$('#' + txtGLAccCodeID).val('');
        //    var detRow = $('#' + txtItemID).closest('tr.gridRow');
        //    $(detRow).find('input[id$="hdnFromDepartmentID"]').val('0');
        //    $(detRow).find('input[id$="txtDeptName"]').val('');
           
        //}
        //function SetDeptData(txtItemCodeID, data) {
    ...
    <div id="dvPageContent" style="width: 100%; height: auto;">
        <div id="dvContentHeader" class="dvContentHeader">
            <div id="dvHeader" class="dvHeader_Prod">
                <asp:Label ID="lblHeader" CssClass="lblHeader" runat="server" Text="Assembly Plate Rejection Declaration"></asp:Label>
            </div>
            <!--Message Div -->
        ...
        <div id="dvContentMain" class="dvContentMain">
            <div id="dvControlsHead" style="height: auto; width: 100%;">
                <table>
                    ...
                    <tr>--%>
                        ...
                        <td align="right" class="auto-style2">
                            <asp:Label ID="lblRejectionNo" runat="server" Text="Rejection NO :" style="font-weight: 700"></asp:Label>
                        </td>
                        <td class="auto-style2">
                            <asp:TextBox ID="txtRejectionNo" runat="server" Style="text-align: left; font-weight: 700;" CssClass="textBox" ForeColor="Red"  Enabled="False"></asp:TextBox>
                        </td>
                        <td align="right">
                              <asp:Label ID="lblRejectionDate" runat="server" Text="Rejection Date :" style="font-weight: 700"></asp:Label>
                        </td>
                        <td>
                              <asp:TextBox ID="txtRejectionDate" runat="server" CssClass="textBox textDate dateParse"  onkeypress=" return isDateValidKey(event,this);" Style="text-align: left;" Width="100px"></asp:TextBox>
                        </td>
                    </tr>
                    <tr>
                        <td style="text-align: right">
                                  <asp:Label ID="lblDEPT_ID" runat="server" Text="Department : " Font-Bold="true"></asp:Label>
                        </td>
                        ...
                        <td align="right">
                            <asp:Label ID="lblProductionDate" runat="server" Text="Production Date :" style="font-weight: 700; display:none;"></asp:Label>
                             <asp:Label ID="Label1" runat="server" Text="Shift : " style="font-weight: 700"></asp:Label>
                        </td>
                        <td>
                             <asp:DropDownList ID="ddlSHIFT_ID" runat="server" CssClass="dropDownList" Width="155px" TabIndex="3" ></asp:DropDownList>
                            <asp:TextBox ID="txtProductionDate" runat="server" CssClass="textBox textDate dateParse" Style="text-align: left;display:none;" Width="100px" Visible="false"></asp:TextBox>
                        </td>
                       <%-- <td>
                    ...
                    <tr>
                         <td style="text-align: right;">
                                  <asp:Label ID="lblRejectionItem" style="" runat="server" Text="Rejection Type : : "></asp:Label>
                              </td>
                              <td style="text-align: left;">
                                <%--   <asp:TextBox ID="txtRejectionItem" runat="server" CssClass="textBox" Style="text-align: left;" ></asp:TextBox>
                                  ...
                                  <asp:DropDownList ID="ddlREJECT_ITEM_TYPE" runat="server"  CssClass="dropDownList required" Width="190px" >
                                      <%--<asp:ListItem  Value="Grid">Reject Grid</asp:ListItem>--%>
                                      <asp:ListItem Selected="True" Value="Plate">Reject Plate</asp:ListItem>
                                  </asp:DropDownList>
                                  <asp:DropDownList ID="ddlRejectionItem" runat="server" style="display:none;" CssClass="dropDownList required" Width="190px" OnSelectedIndexChanged="ddlRejectionItem_SelectedIndexChanged" AutoPostBack="True" ></asp:DropDownList>
                              </td>
                         <td style="text-align: right">
                             <asp:Label ID="Label2" runat="server" Text="UOM :" style="font-weight: 700;display:none;"></asp:Label>
                        </td>
                        <td>
                            <asp:TextBox ID="txtREJECT_ITEM_UOM_NAME"  runat="server" CssClass="textBox" Style="text-align: left;display:none;" Width="100px" ></asp:TextBox>
                            <asp:CheckBox ID="chkRECOVERTOREJECT" Text="Recover To Reject" runat="server" />
                        </td>
                    </tr>
                    <tr>
                        <td align="right" class="auto-style2">
                            <asp:Label ID="lblRejectionReason" runat="server" Text="Rejection Reason :" Font-Bold="true"></asp:Label>
                        </td>
                        <td class="auto-style2" colspan="4">
                            <asp:TextBox ID="txtRejectionReason" runat="server" CssClass="textBox" Style="text-align: left;" Width="500px" ></asp:TextBox>
                        </td>
            ...
            <div id="dvControls" style="width: 100%; height: 400px;">
                ...
                <div id="Div1" runat="server" class="" style="width: auto; height: auto; text-align: left">
                    <div id="dvGridHeaderClosing" style="width: 1000px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                        <table style="height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                            ...
                            <tr class="headerRow_Prod">
                                ...
                                <td width="15px" class="headerColLeft"></td>
                                 ...
                                 <td width="70px" class="headerColCenter_prod">Std. Wt.(Kg/Pc)
                                </td>
                               <%--  <td width="80px" class="headerColCenter_prod">Production Qty
                                </td>--%>
                                <td width="80px" class="headerColCenter_prod">Reject Qty
                                </td>
                                 <%--<td width="80px" class="headerColCenter_prod">Good Qty
                                </td>--%>
                                <td width="80px" class="headerColCenter_prod">Reject Weight
                                </td>
                                                    ...
                                                    <table border="0" cellpadding="1" cellspacing="1">
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <asp:TextBox ID="txtDeptName" runat="server" CssClass="textBox textAutoSelect" Width="130px" align="right" Text='<%# Bind("DEPARTMENT_NAME") %>'></asp:TextBox>
                                                                    <asp:HiddenField ID="hdnFromDepartmentID" runat="server" Value='<%# Bind("FROM_DEPT_ID") %>' />
                                                                </td>
                                                                 <td>
                                                                   <input id="btnDeptID" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                </td>
                                                                <td>
                                                                    <asp:HiddenField ID="hdnPROD_REJECTION_ID" runat="server" Value='<%# Bind("PROD_ASM_REJ_ID") %>' />
                                                                </td>
                                                                <td style="display:none;">
                                                                    <asp:TextBox ID="txtBatchNo" runat="server" CssClass="textBox textAutoSelect" Width="119px" align="right" Text='<%# Bind("BATCH_NO") %>'></asp:TextBox>
                                                                   <%-- <asp:HiddenField ID="hdnBatchNo" runat="server" Value='<%# Bind("BATCH_NO") %>' />--%>
                                                                </td>
                                                                 <td style="display:none;">
                                                                   <input id="btnBatchNo" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                </td>
                                                                <td>
                                                                    <asp:TextBox ID="txtITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="128px" Text='<%# Bind("ITEM_NAME") %>'></asp:TextBox>
                                                                ...
                                                                <td>
                                                                  ...
                                                                  <td>
                                                                    ...
                                                                    <asp:TextBox ID="txtUOM_NAME" runat="server" CssClass="textBox textAutoSelect"  Width="68px" Text='<%# Bind("UOM_NAME") %>'></asp:TextBox>
                                                                    <asp:HiddenField ID="hdnUOM_ID" runat="server" Value='<%# Bind("UOM_ID") %>' />
                                                                </td>
                                                                
                                                                <td>

                                                                    <asp:TextBox ID="txtITEM_STANDARD_WEIGHT_KG" runat="server" CssClass="textBox textAutoSelect"  Width="65px" Text='<%# Eval("ITEM_STANDARD_WEIGHT_KG", "{0:#,##0.0000}") %>'></asp:TextBox>
                                                                    <asp:HiddenField ID="hdnPANEL_PC" runat="server" Value='<%# Bind("PANEL_PC") %>' />
                                                                </td>
                                                                 <%--
                                                                <td>
                                                                    <asp:TextBox ID="txtPRODUCTION_QTY" runat="server" CssClass="textBox textAutoSelect " style="text-align: right;"   Width="75px" align="right" Text='<%# Bind("PRODUCTION_QTY") %>'   onkeypress=" return isNumberKey(event,this);"></asp:TextBox>
                                                                </td>--%>

                                                                 <td>
                                                                    <asp:TextBox ID="txtRejection_QTY" runat="server" style="text-align: right;" CssClass="textBox textAutoSelect txtQty" BackColor="Khaki"   Width="75px" align="right" Text='<%# Bind("REJECTION_QTY") %>' onchange="calcWeight(this)" onkeypress=" return isNumberKey(event,this);"></asp:TextBox>
                                                                </td>

                                                                
                                                                <%--<td>
                                                                    <asp:TextBox ID="txtGOOD_QTY" runat="server" CssClass="textBox textAutoSelect " style="text-align: right;"  Width="75px" align="right" Text='<%# Bind("GOOD_STOCK_QTY") %>'  onkeypress=" return isNumberKey(event,this);"></asp:TextBox>
                                                                </td>--%>
                                                                <td>
                                                                    <asp:TextBox ID="txtREJECTION_WEIGHT" runat="server" style="text-align: right;" CssClass="textBox textAutoSelect txtWt"   Width="77px" align="right" Text='<%# Eval("REJECTION_WEIGHT", "{0:#,##0.00}") %>'    onkeypress=" return isNumberKey(event,this);"></asp:TextBox>
                                                                </td>
                                                                <td>
                                                                    <asp:TextBox ID="txtREJECTION_DET_REMARKS" runat="server" CssClass="textBox textAutoSelect" Width="144px" Text='<%# Bind("REJECTION_DET_REMARKS") %>' Style=""></asp:TextBox>
                                                                </td>
