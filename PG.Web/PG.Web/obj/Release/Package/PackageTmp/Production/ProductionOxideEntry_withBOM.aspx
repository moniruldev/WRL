<%@ Page Title="" Language="C#" EnableEventValidation="false" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="ProductionOxideEntry_withBOM.aspx.cs" Inherits="PG.Web.Production.ProductionOxideEntry_withBOM" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script language="javascript" type="text/javascript">
      ...
      <%--  var Get_ItemWiseBatchNoStock_List = '<%= this.Get_ItemWiseBatchNoStock_List%>';--%>
       ...
       <%-- var txtSUPERVISOR_NAME = '<%= txtSUPERVISOR_NAME.ClientID%>';
            ...
            pageInstance.add_pageLoaded(function (sender, args) {
                ...
                for (i = 0; i < panels.length; i++) {
                    ...
                    if (panels[i].id == gridClosingUpdatePanelIDDet) {
                        ...
                        bindItemWiseBatchStockList(gridViewClosingIDDet);
        ...
        //Batch Stock List
        function bindItemWiseBatchStockList(gridViewIDD) {
            var cgColumns = [{ 'columnName': 'batchno', 'width': '80', 'align': 'left', 'highlight': 4, 'label': 'Batch' }
                             , { 'columnName': 'itemname', 'width': '200', 'align': 'left', 'highlight': 4, 'label': 'Name' }
                             , { 'columnName': 'closingqty', 'width': '80', 'align': 'left', 'highlight': 4, 'label': 'Closing Qty' }
            ];
            var serviceURL = Get_ItemWiseBatchNoStock_List + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            //serviceURL += "&companyid=" + companyid;
            serviceURL += "&ispaging=1&deptid=136&isFinished=N&for_production=Y";
            ...
            $(gridSelector).find('input[id$="txtBatchNo"]').each(function (index, elem) {
                ...
                $(elem).combogrid({
                    ...
                    search: function (event, ui) {
                        ...
                        var vitemid = $(elemRowCur).find('input[id$="hdnCLOSING_ITEM_ID"]').val();
                        //var vgroupid = $('#' + hdngroupId).val();
                        var newServiceURL = serviceURL + "&typeid=" + vitemid;
                    ...
                    select: function (event, ui) {
                        ...
                        if (!ui.item) {
                            ...
                            ClearBatchStockItemData(elemID);
                            return false;
                        }
                        if (ui.item.id == 0) {
                            event.preventDefault();
                            ...
                            SetBatchStcokItemData(elemID, ui.item);
                        }
                        return false;
                    }
                });
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        if (prcNo == null) {
                            ...
                            SetBatchStcokItemData(elemID, grp);
                        }
        ...
        function ClearBatchStockItemData(txtCLOSINGITEM_NAME) {
            //$('#' + txtGLAccCodeID).val('');
            var detRow = $('#' + txtCLOSINGITEM_NAME).closest('tr.gridRow');
            $(detRow).find('input[id$="txtBatchNo"]').val('0');
            $(detRow).find('input[id$="txtSYSTEM_OPENING_STOCK"]').val('0');
            

        }
        function SetBatchStcokItemData(txtCLOSINGITEM_NAME, data) {
            //$('#' + hdnCLOSING_ITEM_ID).val(data.itemid);
            var detRow = $('#' + txtCLOSINGITEM_NAME).closest('tr.gridRow');
            $(detRow).find('input[id$="txtBatchNo"]').val(data.batchno);
            $(detRow).find('input[id$="txtSYSTEM_OPENING_STOCK"]').val(data.closingqty);
        ...
        function bindItemList(gridViewID) {
            var cgColumns = [
                               { 'columnName': 'itemname', 'width': '200', 'align': 'left', 'highlight': 4, 'label': 'Name' }
                               , { 'columnName': 'itemcode', 'width': '80', 'align': 'center', 'highlight': 4, 'label': 'Code' }
            ...
            serviceURL += "&ispaging=1&isFinished=Y&deptid=2&for_production=Y&stlid=2";
            ...
            $(gridSelector).find('input[id$="txtITEM_NAME"]').each(function (index, elem) {
                ...
                $(elem).blur(function () {
                    ...
                    if (eCode == '') {
                        ...
                        var serviceURL = BOMNameListServiceLink + "?isterm=1&deptid=136&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
                        serviceURL += "&ispaging=1";
        ...
        function bindFinishInRawMaterialSection(gridViewID) {
            ...
            var serviceURL = BOMNameListServiceLink + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            serviceURL += "&ispaging=1&isFinished=Y&deptid=136&for_production=Y";
        ...
        function GetItemNo(eCode, serviceURL) {
            ...
            var newServiceURL = serviceURL + "&deptid=136&selectedId=" + eCode;
            var dummyVar = $.ajax({
                type: "GET",
                cache: false,
                async: false,
        ...
        function SetItemData(txtItemCodeID, data) {
            ...
            $(detRow).find('input[id$="txtITEM_CODE"]').val(data.itemcode);
        ...
        function AddAutoRM(texbx)
        {
            ...
            $(detRow).find('input[id$="txtItem_qty"]').attr("readOnly", "readOnly");
            $("[id*=btnAddAutoUsedItem]").click();
            //$("[id*=btnAddOutDrossPer]").click();
        ...
        function TestOnTextChange(texbx) {
            ...
            var finishqty = $(detRow).find('input[id$="txtItem_qty"]').val();
            //var tpcqty = pcqty * panelqty;
            //$(detRow).find('input[id$="txtItem_qty"]').val(tpcqty);
            //var standardweight = $(detRow).find('input[id$="hdnITEM_STANDARD_WEIGHT_KG"]').val();
            //var tweight = tpcqty * standardweight;
            //$(detRow).find('input[id$="txtITEM_WEIGHT"]').val(tweight);
            $('#' + hdnFinishedQty).val(finishqty);
             ...
             $(gridSelector).find('input[id$="txtOPERATOR_NAME"]').each(function (index, elem) {
                 ...
                 $(elem).combogrid({
                     ...
                     search: function (event, ui) {
                         ...
                         //" + dept_id + "
                         var newServiceURL = serviceURL + "&deptid=136&name=" + name;
                         newServiceURL = JSUtility.AddTimeToQueryString(newServiceURL);
                         $(this).combogrid("option", "url", newServiceURL);
                 ...
                 $(elem).blur(function () {
                     ...
                     if (eCode == '') {
                         ...
                         var dept_id = e.options[e.selectedIndex].value;
                         var serviceURL = SupporvisorListServiceLink + "?isterm=1&includeempty=0&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
                         serviceURL += "&ispaging=0&isOperator=1&deptid=136" ;
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="dvPageContent" style="width: 100%; height: 100%;" onkeydown="if(event.keyCode==13){event.keyCode=9; return event.keyCode;}">
        ...
        <asp:Button ID="btnAddAutoUsedItem" runat="server" Style="display: none;" OnClick="btnAddAutoUsedItem_Click" />
         <%--<asp:Button ID="btnAddOutDrossPer" runat="server" Style="display: none;" OnClick="btnAddOutDrossPer_Click" />--%>
        ...
        <asp:HiddenField ID="hdnStoragelocationID" runat="server" Value="2" />
        ...
        <div id="dvContentHeader" class="dvContentHeader">
            <div id="dvHeader" class="dvHeader_Prod" align="left">
                <asp:Label ID="lblHeader" runat="server" Text="Production OXIDE Entry" CssClass="lblHeader" Font-Bold="true" Font-Size="15px"></asp:Label>
            </div>
...
<%--    <li><a href="#tabs-3">Dross </a></li>
                ...
                <table border="0" cellspacing="4" cellpadding="2" align="left" style="width: 90%" id="tblProductionMstEntry">
                    <tr>
                        ...
                        <td style="text-align: left">
                            ...
                            <asp:DropDownList ID="ddlDEPT_ID" runat="server" CssClass="dropDownList required" Width="200px"></asp:DropDownList>
                        </td>
                        <%--<td style="text-align: right">
                            <asp:Label ID="lblSUPERVISOR_ID" runat="server" Text="Supervisor : " Visible="false"></asp:Label>
                        </td>
                        <td style="text-align: left">
                            <asp:TextBox ID="txtSUPERVISOR_NAME" runat="server" CssClass="textBox textAutoSelect" Width="150px"  TabIndex="1"  autofocus Visible="false"></asp:TextBox>
                            <input id="btnSUPERVISOR_ID" type="button" value="" runat="server" class="buttonDropdown" style="width: 15px" tabindex="-1" Visible="false"/>
                        </td>--%>
                        <td>&nbsp;</td>
                    ...
                    <tr>
                        ...
                        <td style="text-align: left">
                            ...
                            <asp:TextBox ID="txtPROD_BATCH_NO" runat="server" CssClass="textBox textAutoSelect textNumberOnly" Width="163px" TabIndex="2"></asp:TextBox>
                        </td>
                       <%-- <td style="text-align: right">
                        ...
                        <td style="text-align: left">
                            <asp:DropDownList ID="ddlSHIFT_ID" runat="server" CssClass="dropDownList" Width="155px" TabIndex="2" OnSelectedIndexChanged="ddlSHIFT_ID_SelectedIndexChanged"></asp:DropDownList>
                    ...
                    <tr>
                        ...
                        <td>
                             <asp:TextBox ID="TextBox1" runat="server" CssClass="textBox" Style="text-align: left;background-color:gray; color : white;font-size:larger; font-weight: 700;" Text="Oxide" Width="150px"></asp:TextBox>
                        </td>
 ...
 <div id="tabs-1">
            ...
            <div id="groupBoxContainer boxShadow" style="height: auto; width: 100%;">
                 <div id="dvGridHeader2" style="float: left; width: 1200px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                <table style="height: 80%; color: #FFFFFF; font-weight: bold; text-align: left;"
                    ...
                    <tr class="headerRow_Prod">
                        ...
                        <td width="83px" class="headerColCenter_prod">Machine
                        </td>
                        <td width="295px" class="headerColCenter_prod">FG Item
                        </td>
                        <td width="15px" class="headerColCenter_prod"></td>
                       <%-- <td width="55px" class="headerColCenter_prod">Panel Qty
                        </td>
                        <td width="68px" class="headerColCenter_prod">Panel UOM
                        </td>--%>
                        <%-- <td width="15px" class="headerColLeft"></td>--%>
                         <td width="72px" class="headerColCenter_prod">Item Code
                        </td>
                        ...
                        <td width="120px" class="headerColCenter_prod">Operator
                        </td>--%>
                         <td width="170px" class="headerColCenter"> BOM
                                            </td>
                ...
                <div id="groupDataDetails" style="width: 90%; height: auto;">
                    ...
                    <div id="dvGridContainer2" class=" " runat="server" style="height: auto; text-align: left; width: 100%">
                        ...
                        <div id="dvGrid" style="width: 1250px; height: 280px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
                                                        ...
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="290px" Text='<%# Bind("ITEM_NAME") %>'></asp:TextBox>
                                                                   ...
                                                                   <td align="right">
                                                                        <asp:TextBox ID="txtITEM_CODE" runat="server" CssClass="textBox textAutoSelect" Style="text-align: right;" Width="70px" BackColor="Khaki" Font-Bold="true" align="right" Text='<%# Bind("ITEM_CODE") %>'></asp:TextBox>
                                                                    </td>

                                                                   <%--  <td>
                                                                        <asp:TextBox ID="txtPANEL_UOM_ID" runat="server" CssClass="textBox textAutoSelect" Width="50px" Text='<%#Bind("PANEL_UOM_NAME") %>'></asp:TextBox>
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtItem_qty" runat="server" CssClass="txtQty textBox textAutoSelect" BackColor="Yellow" Width="57px" align="right" Style="text-align: right;" Text='<%# Bind("ITEM_QTY") %>' onchange="TestOnTextChange(this)" onBlur="AddAutoRM(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    ...
                                                                    <td  style="display : none;">
                                                                     ...
                                                                     <td>
                                                                                <asp:TextBox ID="txtBOM_Name" runat="server" CssClass="textBox textAutoSelect" Width="170px" Text='<%#Bind("BOM_NAME") %>'></asp:TextBox>
                                ...
                                <Triggers>
                                    <asp:AsyncPostBackTrigger ControlID="btnNewRow" EventName="Click" />
                                     <asp:AsyncPostBackTrigger ControlID="btnAddAutoUsedItem" EventName="Click" />
                                </Triggers>
                            </asp:UpdatePanel>
                        </div>
                                ...
                                <tr>
                                    ...
                                    <td style="width: 140px; text-align: right;" align="right">
                                        <%--<asp:TextBox ID="txtPanelWeight" runat="server" Width="90px" Text="0" CssClass="textBoxReadOnly"></asp:TextBox>--%>
                                          <asp:TextBox ID="txtTotalPanelQty" runat="server" Width="180px" Text="0" ForeColor="Red" Font-Size="Medium" CssClass="textBoxReadOnly"></asp:TextBox>
                                    </td>
                                    <td align="right">&nbsp;</td>
                                    <td align="right" style="width: 10px">&nbsp;
                                    </td>
                                    <td style="width: 90px" align="left">
                                        <asp:Button ID="btnNewRow" runat="server" CssClass="buttonNewRow" Visible="true" Text="" OnClientClick="ShowProgress()" OnClick="btnNewRow_Click" />
           ...
           <div id="groupBoxContainer1 boxShadow" style="height: auto; ">
                <div id="groupDataDetails1" style="width: 100%; height: auto;">
                    <div id="Div1" runat="server" class="" style="width: auto; height: auto; text-align: left; width: 100%">
                        <div id="dvGridHeaderClosing" style="float:left; width: 1250px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                ...
                                <tr class="headerRow_Prod">
                                  ...
                                  <%--  <td width="180px" class="headerColCenter_prod">Finished Item
                                    ...
                                    <td width="205px" class="headerColCenter_prod"> RM Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>
                                     <td width="65px" class="headerColCenter_prod">Item Code
                                    </td>
                                    ...
                                    <td width="75px" class="headerColCenter_prod">Closing Qty
                                     ...
                                     <td width="70px" class="headerColCenter_prod">Batch No
                                    </td>
                                     <td width="15px" class="headerColCenter_prod"></td>
                        ...
                        <div id="dvGridClosing" style=" float:left; width: 1150px; height: 250px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel2" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grdClosingRowMaterial" runat="server" width="1200px" AutoGenerateColumns="False" ShowHeader="False"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderText="SL#" >
                                                <ItemTemplate>
                                                    <asp:Label ID="lblCLOSING_SI" runat="server" Text= <%# Container.DataItemIndex + 1 %>   Style="text-align: center;" Width="35px">
                                                    </asp:Label>
                                            ...
                                            <asp:TemplateField HeaderText="SL#" >
                                                <ItemTemplate>
                                                  <asp:LinkButton ID="btnLinkSC" runat="server" CssClass="buttoncommon" CommandName="ADD" Width="20px" height="17px" Font-Size="Medium"  Font-Bold="true" >+</asp:LinkButton>

                                                </ItemTemplate>
                                                <ItemStyle VerticalAlign="Top" Width="14px" />
                                            </asp:TemplateField>
                                                        ...
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtRM_ITEM_CODE" runat="server" CssClass="textBox textAutoSelect"  Width="65px" style="text-align:right;" Text='<%# Bind("RM_ITEM_CODE") %>'></asp:TextBox>
                                                                    </td>
                                                                    <td>
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtSYSTEM_OPENING_STOCK" runat="server" CssClass="textBox textAutoSelect"  Width="100px" style="text-align:right;" Text='<%# Eval("SYSTEM_OPENING_STOCK", "{0:#,##0.00}") %>' onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <%-- <td >
                                                                    ...
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtCLOSING_QTY" runat="server" CssClass="textBox textAutoSelect" Width="70px" style="text-align:right;" Text='<%# Eval("CLOSING_QTY", "{0:#,##0.00}") %>' onchange="calcIssueStock(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                     <td align="right">
                                                                        <asp:TextBox ID="txtBatchNo" runat="server" CssClass="textBox textAutoSelect" Width="70px" style="text-align:right;" Text='<%# Bind("PROD_BATCH_NO") %>'></asp:TextBox>
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnBatchNo" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>
                                                                    ...
                                                                    <td align="right" style="display:none">
                                                                        <asp:TextBox ID="txtPOSITIVE_DEV" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="60px" style="text-align:right;" BackColor="Khaki"  Text='<%# Eval( "POSITIVE_DEV", "{0:#,##0.00}") %>'  onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox>
                                                                    </td>
                                                                    <td align="right" style="display:none">
                                                                        <asp:TextBox ID="txtNEGATIVE_DEV" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="60" style="text-align:right;" BackColor="Khaki"  Text='<%# Eval("NEGATIVE_DEV", "{0:#,##0.00}") %>' onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox> 
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtISSUE_STOCK" runat="server" CssClass="textBox textAutoSelect" Width="72" BackColor="Yellow"   align="right" style="text-align:right;"  Text='<%# Eval("ISSUE_STOCK", "{0:#,##0.00000}") %>' onchange="calcCloseingStock(this)" onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td>
                                                             ...
                                                             <tr style="display:none;">
                                                                    <td colspan="5">
                                                                        <asp:TextBox ID="txtFINISH_ITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="280px" Text='<%# Bind("FINISH_ITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnFINISHED_ITEM_ID" runat="server" Value='<%# Bind("FINISHED_ITEM_ID") %>' />
                                                                 </td>
                                                                 <td>
                                                                      <input id="btnFinishItem" type="button" value="" runat="server" class="buttonDropdown" style="display:none;" tabindex="-1" />
                                                                 </td>
                                                               <td></td>
                                                               <td></td>
                                                               <td></td>
                    ...
                    <div id="groupBoxContainer2 boxShadow" style="height: auto; width: 100%;">
                            <div id="groupDataDetails2" style="float: left;width: 90%; height: auto;">
                                ...
                                <div id="Div7" runat="server" class="" style=" float:left; width: auto; height: auto; text-align: left; width: 100%">
                                    <div id="dvGridHeaderClosingSummary" style="width: 635px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                                        <table style="height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                            class="defFont" cellspacing="1" cellpadding="1">
                                            <tr class="headerRow_Prod">
      ...
      <div id="tabs-3">
            ...
            <%--
                    <div id="Div2" runat="server" class="" style="float:left; width: 100%; text-align: left;  border-top: solid 1px #0b07f5;">
                        <span style="font-weight: bold; font-size: 15; color: #ff3b00;">Dross Details: </span>
                    </div>
           <div id="groupBoxContainer3 boxShadow" style="height: auto; ">
                <div id="groupDataDetails3" style="width: 100%; height: auto;">
                    <div id="Div4" runat="server" class="" style="width: auto; height: auto; text-align: left; width: 100%">
                        <div id="dvGridHeaderDross" style="float:left; width: 1000px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                class="defFont" cellspacing="1" cellpadding="1">
                                <tr class="headerRow_Prod">
                                    <td width="40px" class="headerColCenter_prod">SL#
                                    ...
                                    <td width="65px" class="headerColCenter_prod">UOM
                                     ...
                                     <td width="75px" class="headerColCenter_prod">Std Dross Qty
                                    </td>
                            
                                    <td width="62px" class="headerColCenter_prod">Dross Qty
                                    </td>
                        ...
                        <div id="dvGridDross" style=" float:left; width: 1400px; height: 250px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel4" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grdvDross" runat="server" width="1200px" AutoGenerateColumns="False" ShowHeader="False"
                                        CellPadding="1" CellSpacing="1" ForeColor="#333333" GridLines="Both" DataKeyNames="PROD_DROSS_ID"
                                        EnableModelValidation="True" ClientIDMode="AutoID" OnRowCommand="grdvDross_RowCommand" OnRowCreated="grdvDross_RowCreated" OnRowDataBound="grdvDross_RowDataBound" OnRowDeleting="grdvDross_RowDeleting">
                                        <RowStyle BackColor="#F7F6F3" ForeColor="#333333" />
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderText="SL#" >
                                                <ItemTemplate>
                                                    <asp:Label ID="lblDROSS_SI" runat="server" Text= <%# Container.DataItemIndex + 1 %>   Style="text-align: center;" Width="40px">
                                                    </asp:Label>
                                                    ...
                                                    <div>
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_DROSS_ID" runat="server" Value='<%# Bind("PROD_DROSS_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_MST_ID" runat="server" Value='<%# Bind("PROD_MST_ID") %>' />
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtFINISH_ITEM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="176px" Text='<%# Bind("FINISH_ITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnFINISHED_ITEM_ID" runat="server" Value='<%# Bind("FINISHED_ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnFinishItem" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtDrossItem_NAME" runat="server" CssClass="textBox textAutoSelect" Width="198px" Text='<%# Bind("DROSSITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnDROSS_ITEM_ID" runat="server" Value='<%# Bind("DROSS_ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnDROSSITEM_NAME" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>
                                                                    <td>

                                                                        <asp:TextBox ID="txtDROSS_UOM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="62px" Text='<%# Bind("DROSS_UOM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnDROSS_UOM_ID" runat="server" Value='<%# Bind("DROSS_UOM_ID") %>' />
                                                                    </td>
                    
                                                                    
                                                                        <td align="right">
                                                                        <asp:TextBox ID="txtSTD_DROSS_QTY" runat="server" CssClass="textBox textAutoSelect" Width="72"  align="right" style="text-align:right;" Text='<%# Eval( "STD_DROSS_QTY", "{0:#,##0.00}") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtDROSS_ITEM_QTY" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="60px" style="text-align:right;" BackColor="Yellow"  Text='<%# Eval( "DROSS_ITEM_QTY", "{0:#,##0.00}") %>'  onBlur=" calcCloseingDevStock(this)" onchange="TestOnTextChangegqty(this)" onkeypress="return isNumberKey(event,this);" ></asp:TextBox>
                                                                    </td>
                                                                   
                                                                    <td>
                                                                        <asp:TextBox ID="txtDROSSDTL_REMARKS" runat="server" CssClass="textBox textAutoSelect" Width="110px" Text='<%# Bind("DROSS_DTL_REMARKS") %>' Style=""></asp:TextBox>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                ...
                                <Triggers>
                                    <asp:AsyncPostBackTrigger ControlID="btnNewdross" EventName="Click" />
                                </Triggers>
                            </asp:UpdatePanel>
                        </div>

                        <div id="divGridControlsDross">
                            <table style="width: 70%; height: 100%; text-align: center;" cellspacing="1" cellpadding="1"
                                border="0">
                                <tr>
                                    <td style="width: 2px"></td>
                                    ...
                                    <td style="width: 90px" align="left">
                                        <asp:Button ID="btnNewdross" runat="server" CssClass="buttonNewRow" Text="" OnClientClick="ShowClosingProgress()" OnClick="btnNewdross_Click" />
                                    </td>
                                    <td style="width: 20px;">
                                        <asp:UpdateProgress ID="UpdateProgress4" runat="server" AssociatedUpdatePanelID="UpdatePanel4"
                                            DisplayAfter="300">
                                            <ProgressTemplate>
                                                <asp:Image ID="imgProgressClosing4" runat="server" ImageUrl="~/image/loader.gif" />
                                            </ProgressTemplate>
                                        </asp:UpdateProgress>
                                    </td>
                                </tr>
          ...
          <div id="tabs-4">
                    <%-- Start Cutting Details --%>

             <%--
                    <div id="Div8" runat="server" class="" style="float:left; width: 100%; text-align: left;  border-top: solid 1px #0b07f5;">
                        <span style="font-weight: bold; font-size: 15; color: #ff3b00;">Cutting Details: </span>
                    </div>
           <div id="groupBoxContainer4 boxShadow" style="height: auto; ">
                <div id="groupDataDetails4" style="width: 100%; height: auto;">
                    <div id="Div9" runat="server" class="" style="width: auto; height: auto; text-align: left; width: 100%">
                        <div id="dvGridHeaderCutting" style="float:left; width: 1000px; height: 25px; font-size: smaller;" class="subHeader_Prod">
                            <table style=" height: 100%; color: #FFFFFF; font-weight: bold; text-align: left;"
                                class="defFont" cellspacing="1" cellpadding="1">
                                <tr class="headerRow_Prod">
                                    <td width="40px" class="headerColCenter_prod">SL#
                                    ...
                                    <td width="15px" class="headerColCenter_prod"></td>
                                    <td width="200px" class="headerColCenter_prod"> Cutting Item
                                    </td>
                                    <td width="15px" class="headerColCenter_prod"></td>
                                    ...
                                    <td width="65px" class="headerColCenter_prod">UOM
                                     ...
                                     <td width="75px" class="headerColCenter_prod">Std Cutting Qty
                                    </td>
                            
                                    <td width="62px" class="headerColCenter_prod">Cutting Qty
                                    </td>
                        ...
                        <div id="dvGridCutting" style=" float:left; width: 1400px; height: 250px; overflow: auto;" class="dvGrid">
                            <asp:UpdatePanel ID="UpdatePanel5" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grdvCutting" runat="server" width="1200px" AutoGenerateColumns="False" ShowHeader="False"
                                        CellPadding="1" CellSpacing="1" ForeColor="#333333" GridLines="Both" DataKeyNames="PROD_CUTTING_ID"
                                        EnableModelValidation="True" ClientIDMode="AutoID" OnRowCommand="grdvCutting_RowCommand" OnRowCreated="grdvCutting_RowCreated" OnRowDataBound="grdvCutting_RowDataBound" OnRowDeleting="grdvCutting_RowDeleting">
                                        <RowStyle BackColor="#F7F6F3" ForeColor="#333333" />
                                        <Columns>
                                                    ...
                                                    <div>
                                                        <table border="0" cellpadding="1" cellspacing="1">
                                                            <tbody>
                                                                <tr>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_CUTTING_ID" runat="server" Value='<%# Bind("PROD_CUTTING_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <asp:HiddenField ID="hdnPROD_MST_ID" runat="server" Value='<%# Bind("PROD_MST_ID") %>' />
                                                                    ...
                                                                    <td>
                                                                        <asp:TextBox ID="txtCuttingItem_NAME" runat="server" CssClass="textBox textAutoSelect" Width="198px" Text='<%# Bind("CUTTINGITEM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnCUTTING_ITEM_ID" runat="server" Value='<%# Bind("CUTTING_ITEM_ID") %>' />
                                                                    </td>
                                                                    <td>
                                                                        <input id="btnCuttingITEM_NAME" type="button" value="" runat="server" class="buttonDropdown" tabindex="-1" />
                                                                    </td>
                                                                    <td>

                                                                        <asp:TextBox ID="txtCutting_UOM_NAME" runat="server" CssClass="textBox textAutoSelect" Width="62px" Text='<%# Bind("CUTTING_UOM_NAME") %>'></asp:TextBox>
                                                                        <asp:HiddenField ID="hdnCUTTING_UOM_ID" runat="server" Value='<%# Bind("CUTTING_UOM_ID") %>' />
                                                                    </td>
                    
                                                                    
                                                                        <td align="right">
                                                                        <asp:TextBox ID="txtSTD_CUTTING_QTY" runat="server" CssClass="textBox textAutoSelect" Width="72"  align="right" style="text-align:right;" Text='<%# Eval( "STD_CUTTING_QTY", "{0:#,##0.00}") %>'  onkeypress="return isNumberKey(event,this);"></asp:TextBox>
                                                                    </td>
                                                                    <td align="right">
                                                                        <asp:TextBox ID="txtCUTTING_ITEM_QTY" runat="server" CssClass="txtgQty textBox textAutoSelect"   Width="60px" style="text-align:right;" BackColor="Yellow"  Text='<%# Eval( "CUTTING_ITEM_QTY", "{0:#,##0.00}") %>'  onkeypress="return isNumberKey(event,this);" ></asp:TextBox>
                                                                    </td>
                                                           
                                                                    <td>
                                                                        <asp:TextBox ID="txtCUTTING_DTL_REMARKS" runat="server" CssClass="textBox textAutoSelect" Width="110px" Text='<%# Bind("CUTTING_DTL_REMARKS") %>' Style=""></asp:TextBox>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                    ...
                                    <input id="Hidden7" type="hidden" runat="server" value="[]" />
                                    <input id="Hidden8" type="hidden" runat="server" value="[]" />
                                </ContentTemplate>
                                <Triggers>
                                    <asp:AsyncPostBackTrigger ControlID="btnNewCutting" EventName="Click" />
                                </Triggers>
                            </asp:UpdatePanel>
                        </div>

                        <div id="divGridControlsCutting">
                            <table style="width: 70%; height: 100%; text-align: center;" cellspacing="1" cellpadding="1"
                                border="0">
                                <tr>
                                    <td style="width: 2px"></td>
                                    ...
                                    <td style="width: 90px" align="left">
                                        <asp:Button ID="btnNewCutting" runat="server" CssClass="buttonNewRow" Text="" OnClientClick="ShowClosingProgress()" OnClick="btnNewCutting_Click" />
                                    </td>
                                    <td style="width: 20px;">
                                        <asp:UpdateProgress ID="UpdateProgress5" runat="server" AssociatedUpdatePanelID="UpdatePanel4"
                                            DisplayAfter="300">
                                            <ProgressTemplate>
                                                <asp:Image ID="imgProgressClosing5" runat="server" ImageUrl="~/image/loader.gif" />
                                            </ProgressTemplate>
                                        </asp:UpdateProgress>
                                    </td>
                                </tr>
