<%@ Page Title="" Language="C#" MasterPageFile="~/AppMaster.Master" AutoEventWireup="true" CodeBehind="CargoReceivedMst.aspx.cs" Inherits="PG.Web.WREL.CargoReceivedMst" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script language="javascript" type="text/javascript">
       ...
       <%-- var CargoListService = '<%=this.CargoListService%>';
        ...
        var CargoReceivePendingListServiceLink = '<%=this.CargoReceivePendingListServiceLink%>';
        ...
        var txtContactPerson = '<%=txtContactPerson.ClientID%>';
        var txtMobileNo = '<%=txtMobileNo.ClientID%>';
        var hdnCargoTrackingID = '<%=hdnCargoTrackingID.ClientID%>';
        ...
        function bindCargopendingList() {
            var cgColumns = [
                             { 'columnName': 'cargonumber', 'width': '100', 'align': 'left', 'highlight': 4, 'label': 'Code' }
                           , { 'columnName': 'fhubname', 'width': '100', 'align': 'left', 'highlight': 4, 'label': 'From Hub' }
                           , { 'columnName': 'medianame', 'width': '100', 'align': 'left', 'highlight': 4, 'label': 'Transport' }
                           
            ];
            var serviceURL = CargoReceivePendingListServiceLink + "?isterm=1&includeempty=0&hasitem=1&iscodename=1&codecomptype=" + Enums.DataCompareType.StartsWith;
            ...
            $(groupIDElem).combogrid({
                ...
                select: function (event, ui) {
                    ...
                    if (ui.item.dealerid == '') {
                        ...
                        $('#' + hdnCargoTrackingID).val(ui.item.cargotrackingid);
                        $('#' + hdnCargoID).val(ui.item.cargoid);
                        $('#' + txtCargoNo).val(ui.item.cargonumber);

                        $('#' + hdnFromHubID).val(ui.item.fhubid);
                        $('#' + txtFromHubName).val(ui.item.fhubname);
                        $('#' + txtToHubName).val(ui.item.thubname);
                        $('#' + txtTransportMedia).val(ui.item.medianame);
                        
                        $('#' + hdnToHubID).val(ui.item.thubid);
                        $('#' + hdnTransMediaID).val(ui.item.transMediaid);
                        $('#' + txtContactPerson).val(ui.item.contaperson);
                        $('#' + txtMobileNo).val(ui.item.contactno);
     ...
     <div class="container-fluid">
      ...
      <div class="card-body">
            ...
            <div class="row mb-0">
                 ...
                 <div class="col-md-4">
                  <div class="form-group row mb-0">
                    <label for="name" class="col-sm-4 col-form-label-sm">Receive Date :</label>
                    <div class="col-sm-8">
                     <asp:TextBox ID="txtIssuedate" runat="server" CssClass="TextBoxnew textDate dateParse" Width="130px" ></asp:TextBox>
                       &nbsp;&nbsp; <span style="color:red;"><b>Receive</b></span>
                    </div>
                  </div>
                </div>
